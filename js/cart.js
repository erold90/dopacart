...
DC.views = DC.views || {};
(function () {
  var GOAL = 150; // soglia "drop" fittizia per goal-gradient
  ...
  DC.views.cart = function (root) {
    ...
    if (!lines.length) {
      root.innerHTML =
        ...
        '<div style="margin-top:var(--sp-4)"><button class="btn btn-action" id="goShop">Vai allo shopping</button></div></div>';
    ...
    root.innerHTML =
      ...
      '<div style="margin-top:var(--sp-4)">' +
        lines.map(function (l) {
          ...
          return '<div class="cart-item" data-id="' + p.id + '" style="--h:' + p.hue + '">' +
            ...
            '<div class="qty"><button data-dec="' + p.id + '" aria-label="meno">−</button>' +
              '<span class="tnum">' + l.qty + '</span>' +
              '<button data-inc="' + p.id + '" aria-label="più">+</button></div>' +
          '</div>';
        }).join("") +
      '</div>' +
      ...
      '<div class="sticky-cta"><button class="btn btn-action btn-block btn-lg" id="checkout">' +
        'Vai al checkout · ' + DC.fx.euro(total) + '</button></div>';
