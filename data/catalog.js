/* DopaCart — helper catalogo + LAZY LOADER. I dati sono in data/catalog.json (fetch a runtime). */
window.DC = window.DC || {};
DC.loadCatalog = function () {
  if (DC.catalog) return Promise.resolve(DC.catalog);
  return fetch('data/catalog.json', { cache: 'force-cache' }).then(function (r) { return r.json(); }).then(function (j) { DC.catalog = j; return j; });
};
