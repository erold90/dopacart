...
// Normalizza OGNI foto a packshot quadrato su bianco (wsrv.nl) → coerenza totale a prescindere dalla sorgente
DC.imgSrc = function (url, size) { if (!url) return ""; var s = size || 700; return "https://wsrv.nl/?url=" + encodeURIComponent(url) + "&w=" + s + "&h=" + s + "&fit=contain&cbg=white&output=webp"; };
DC.imgTag = function (p, size) { if (!p || !p.img) return ""; return '<img class="pimg" src="' + DC.imgSrc(p.img, size) + '" alt="' + (p.title || "").replace(/"/g, "&quot;") + '" loading="lazy" onload="this.classList.add(\'on\')" onerror="this.remove()">'; };
